/* src/main.js
   Complete frontend behaviour for Shopping app.
   Hindi comments, technical keywords remain in English.
*/
const API_BASE =
...
$(function () {
  ...
  // last loaded products (search/sort)
  let currentProducts = [];
  ...
  // local cart sanitize (remove products no longer present)
  ...
  function updateCartCount() {
    try {
      ...
      if ($("#cartCount").length) {
        ...
        // some templates show Cart(...) button text
          ...
          .on("click", function () {
            ...
            $.ajax({
              ...
              .then(function (resp) {
                ...
                // redirect to login
                $.ajax({ method: "GET", url: "/login.html" }).then(function (resp2) {
        ...
        $.ajax({
          ...
          .then(function (resp) {
            ...
            if (typeof onSuccess === "function") {
              ...
              $.ajax({ method: "GET", url: "/shophome.html" }).then(function (resp2) {
                $("#bodyContainer").html(resp2);
              });
            }
          })
  ...
  function loadProfilePage() {
    ...
    fetch(API_BASE + "/customers/" + encodeURIComponent(uid))
      .then((r) => {
        if (!r.ok) return r.json().then((j) => Promise.reject({ status: r.status, body: j })).catch(() => Promise.reject({ status: r.status, body: null }));
        return r.json();
      })
      .then((resp) => {
      ...
      .catch((err) => {
        // fallback: try listing customers and find match by email/mobile/UserId
        console.warn("Primary profile load failed, trying fallback:", err);
        fetch(API_BASE + "/customers")
          ...
          .then((all) => {
            ...
            if (found) {
              ...
              $("#UserId").val(found.UserId || found.userId || "");
              $("#FirstName").val(found.FirstName || found.firstName || "");
              $("#LastName").val(found.LastName || found.lastName || "");
              $("#Email").val(found.Email || found.email || "");
              $("#Gender").val(found.Gender || found.gender || "");
              $("#Address").val(found.Address || found.address || "");
              $("#PostalCode").val(found.PostalCode || found.postalCode || "");
              $("#State").val(found.State || found.state || "");
              $("#Country").val(found.Country || found.country || "");
              $("#Mobile").val(found.Mobile || found.mobile || "");
            } else {
              alert("Unable to load profile. The id stored for you (" + uid + ") did not match any customer record. Please contact admin or try logging in again.");
            }
          })
          .catch((e) => {
  ...
  function renderProducts(list) {
    ...
    list.forEach(function (value) {
      ...
      const idVal =
        value.id !== undefined ? String(value.id) : value._id ? String(value._id) : "";
      const card =
        '<div class="card m-2 p-2 product-card" style="width:200px;cursor:pointer;" data-id="' +
        idVal +
        '">' +
        '<img src="' +
        img +
        ...
        title +
        '">' +
        '<div class="card-body"><h6 class="card-title small">' +
        title +
        "</h6><p class='card-text small text-muted'>" +
        price +
        "</p></div></div>";
  ...
  function getProducts(categoryName) {
    ...
    $.ajax({ method: "GET", url: url })
      ...
      .catch(function (err) {
        ...
        $("#productCatalog").html('<p class="text-danger p-3">Unable to load products.</p>');
      });
  }
  ...
  function getCategories() {
    $.ajax({ method: "GET", url: API_BASE + "/categories" })
      ...
      .catch(function (err) {
        ...
        $("#categoryList").html('<p class="text-danger p-3">Unable to load categories.</p>');
      });
  }
  ...
  function showProductDetails(productId) {
    $.ajax({ method: "GET", url: API_BASE + "/getproducts" })
      .then(function (products) {
        ...
        const html =
          '<div class="container my-4"><div class="row"><div class="col-md-5">' +
          '<img src="' + fixImageUrl(image) + '" alt="' + title + '" class="img-fluid rounded" style="max-height:500px;width:100%;object-fit:cover;">' +
          "</div><div class='col-md-7'><h2>" + title + "</h2>" +
          "<h4 class='text-success'>" + (price ? "₹" + price : "") + "</h4>" +
          "<p class='text-muted'><strong>Category:</strong> " + category + "</p>" +
          "<p>" + description + "</p>" +
          "<p><strong>Rating:</strong> <span class='stars' style='--rating:" + (Number(rating) || 0) + "'></span> <span class='rating-value'>(" + (Number(rating) || "NA") + ")</span></p>" +
          "<div class='mt-3'><button id='btnAddToCart' data-id='" + idVal + "' class='btn btn-primary me-2'>Add to Cart</button>" +
          "<button id='btnBackToCatalog' class='btn btn-outline-secondary'>Back to Catalog</button></div></div></div></div>";
        $("#bodyContainer").html(html);
      ...
      .then(function (products) {
        const itemsWithProducts = cart.map(function (ci) {
          const prod = products.find(function (p) {
            return (p.id !== undefined && String(p.id) === String(ci.id)) || (p._id !== undefined && String(p._id) === String(ci.id));
          });
          return { prod: prod, qty: ci.qty || 1, cartId: ci.id };
        });

        const missingIds = itemsWithProducts.filter(function (it) { return !it.prod; }).map(function (it) { return String(it.cartId); });
        ...
        const validItems = itemsWithProducts.filter(function (it) { return it.prod; });
        ...
        validItems.forEach(function (it) {
          const p = it.prod;
          html += '<div class="list-group-item d-flex justify-content-between align-items-center"><div><strong>' + (p.title || "") + '</strong><br/><small class="text-muted">' + (p.description || "") + '</small><br/>₹' + (p.price || 0) + ' × ' + it.qty + '</div><div><button class="btn btn-sm btn-outline-secondary btn-decrease" data-id="' + it.cartId + '">−</button> <button class="btn btn-sm btn-outline-secondary btn-increase" data-id="' + it.cartId + '">+</button> <button class="btn btn-sm btn-danger btn-remove" data-id="' + it.cartId + '">Remove</button></div></div>';
        });
        html += '</div><div class="mt-3"><button id="btnCheckout" class="btn btn-primary">Checkout</button> <button id="btnContinue" class="btn btn-link">Continue Shopping</button></div></div>';
        ...
        $(".btn-remove").off("click").on("click", function () {
          ...
          c = c.filter(function (it) { return String(it.id) !== id; });
        ...
        $(".btn-increase").off("click").on("click", function () {
          const id = String($(this).data("id"));
          let c = loadCartForCurrentUser();
          c.forEach(function (it) { if (String(it.id) === id) it.qty = (it.qty || 1) + 1; });
          saveCartToLocalAndUser(c);
          updateCartCount();
          showCart();
        });
        ...
        $(".btn-decrease").off("click").on("click", function () {
          const id = String($(this).data("id"));
          let c = loadCartForCurrentUser();
          c.forEach(function (it) { if (String(it.id) === id) it.qty = Math.max(1, (it.qty || 1) - 1); });
          saveCartToLocalAndUser(c);
          updateCartCount();
          showCart();
        });
        ...
        $("#btnCheckout").off("click").on("click", async function () {
          ...
          try {
            const productsResp = await $.ajax({ method: "GET", url: API_BASE + "/getproducts" });
            const orderItems = currentCart.map(function (ci) {
              const prod = productsResp.find(function (p) {
                return String(p.id) === String(ci.id) || String(p._id) === String(ci.id);
              }) || {};
              const unitPrice = Number(prod.price || 0);
              const qty = ci.qty || 1;
              return { productId: ci.id, title: prod.title || "", unitPrice: unitPrice, qty: qty, lineTotal: unitPrice * qty };
            });

            const badItem = orderItems.find(function (it) { return !it.title && !it.unitPrice; });
            if (badItem) {
            ...
            const subtotal = orderItems.reduce(function (s, it) { return s + it.lineTotal; }, 0);
            const shipping = 0;
            const tax = 0;
            const total = subtotal + shipping + tax;

            if (!window.confirm("You are about to place an order for ₹" + total + ". Proceed?")) return;

            const payload = {
              userId: getCurrentUserId(),
              items: orderItems.map(function (it) { return { productId: it.productId, qty: it.qty, price: it.unitPrice }; }),
              subtotal: subtotal,
              shipping: shipping,
              tax: tax,
              total: total,
            ...
            if (createResp && createResp.success) {
              ...
              updateCartCount();
              $("#bodyContainer").html('<div class="container my-5"><h3>Order Confirmed</h3><p>Order ID: <strong>' + (createResp.orderId || "") + '</strong></p><p>' + (createResp.message || "Thank you for your order!") + '</p><p>Total Paid: ₹' + total + '</p><a href="#" id="btnBackToShop" class="btn btn-primary">Continue Shopping</a></div>');
  ...
  function renderOrderDetails(order) {
    ...
    const itemsRows = Array.isArray(order.items) && order.items.length ? order.items.map(function (it, i) {
      const title = it.title || "Item " + (i + 1);
      const qty = it.qty || 1;
      const price = it.unitPrice || it.price || 0;
      const total = Number(price) * Number(qty);
      return "<tr><td>" + (i + 1) + "</td><td>" + title + "</td><td>" + qty + "</td><td>" + price + "</td><td>" + total + "</td></tr>";
    }).join("") : '<tr><td colspan="5" class="text-center">No items</td></tr>';

    const html = '<div class="container my-4"><h3 class="mb-3">Order Details</h3><div class="card mb-3"><div class="card-body"><p><strong>Order ID:</strong> ' + (order._id || "") + '</p><p><strong>Date:</strong> ' + created + '</p><p><strong>Status:</strong> ' + (order.status || "created") + '</p><p><strong>Total:</strong> ₹' + (order.total || 0) + '</p></div></div><div class="card"><div class="card-body"><h5 class="card-title mb-3">Items</h5><div class="table-responsive"><table class="table table-striped table-bordered align-middle"><thead><tr><th>#</th><th>Item</th><th>Qty</th><th>Price (₹)</th><th>Total (₹)</th></tr></thead><tbody>' + itemsRows + '</tbody></table></div></div></div><div class="mt-3"><button id="btnBackToOrders" class="btn btn-secondary">Back to My Orders</button></div></div>';

    $("#bodyContainer").html(html);
    $("#btnBackToOrders").off("click").on("click", function (e) { e.preventDefault(); showOrders(); });
  }
  ...
  function showOrders() {
    ...
    $.ajax({ method: "GET", url: API_BASE + "/orders/" + encodeURIComponent(uid) })
      .then(function (resp) {
        if (!resp || resp.success === false) {
        ...
        let html = '<div class="container my-4"><h3>My Orders</h3><div class="table-responsive mt-3"><table class="table table-striped table-bordered align-middle"><thead><tr><th>#</th><th>Order ID</th><th>Date</th><th>Items</th><th>Total (₹)</th><th>Status</th><th>Actions</th></tr></thead><tbody>';
        orders.forEach(function (order, idx) {
          const created = order.createdAt ? new Date(order.createdAt).toLocaleString() : "-";
          const itemsText = Array.isArray(order.items) ? order.items.map(function (it) { return (it.title || "Item") + " × " + (it.qty || 1); }).join(", ") : "-";
          html += "<tr><td>" + (idx + 1) + "</td><td>" + (order._id || "") + "</td><td>" + created + "</td><td>" + itemsText + "</td><td>" + (order.total || 0) + "</td><td>" + (order.status || "created") + "</td><td><button class='btn btn-sm btn-outline-primary btn-view-order' data-idx='" + idx + "'>View</button></td></tr>";
        });
  ...
  /* =========================
     Navigation helpers (Shop / Categories)
     ========================= */
    ...
    if (!getCurrentUserId()) {
      $.ajax({ method: "GET", url: "/login.html" }).then(function (resp) {
        ...
        attachLoginHandler(function () {
          $.ajax({ method: "GET", url: "/products.html" }).then(function (resp2) {
            $("#bodyContainer").html(resp2);
            getProducts();
          });
        });
  ...
  $("#btnNavCategories").off("click").on("click", function () {
    if (!getCurrentUserId()) {
      $.ajax({ method: "GET", url: "/login.html" }).then(function (resp) {
        $("#bodyContainer").html(resp);
        attachLoginHandler(function () {
          $.ajax({ method: "GET", url: "/categories.html" }).then(function (resp2) {
            $("#bodyContainer").html(resp2);
            getCategories();
          });
        });
  ...
  // footer links
  $("#navShopF").off("click").on("click", function (e) { e.preventDefault(); $("#btnNavShopping").click(); });
  $("#navRegisterF").off("click").on("click", function (e) { e.preventDefault(); $("#btnNavRegister").click(); });
