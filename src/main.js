...
// Backend base URL (Vite env fallback)
const API_BASE =
...
$(function () {
      ...
      .on("click", async function () {
        ...
        try {
          const resp = await fetch(API_BASE + "/login", {
            ...
            body: JSON.stringify({ UserId: formUserId, Password: formPwd }),
          });
          ...
          if (!resp.ok || !data || data.success === false) {
            const message = (data && data.message) || `Login failed (status ${resp.status})`;
            alert(message);
            return;
          }

          const uid = data.userId || formUserId;
          ...
          alert("Login error (network). Check console for details.");
        }
      });
  }
  ...
  function loadProfilePage() {
    ...
    $("#btnUpdateProfile")
      ...
      .on("click", function (e) {
        ...
        // prefer cookieUser unless user has explicitly typed the same value
        const uidInput = (inputUser && inputUser === cookieUser) ? inputUser : cookieUser || inputUser;
        ...
        fetch(API_BASE + "/customers/" + encodeURIComponent(uidInput), {
          ...
          .then(async (r) => {
            ...
            try {
              ...
              body = { success: false, message: txt || "Server error" };
            }
            return { ok: r.ok, status: r.status, body };
          })
          .then((resObj) => {
            console.log("PROFILE UPDATE RESPONSE →", resObj);
            const up = resObj.body;
            if (resObj.ok && up && up.success) {
              alert(up.message || "Profile updated successfully.");
              $("#Password").val("");
            } else {
              alert((up && up.message) || `Profile update failed (status ${resObj.status}).`);
            }
          })
          .catch((err) => {
  ...
  /* ---- products, categories, cart, orders etc. (same as original) ---- */
  // For brevity I assume your existing functions getProducts/getCategories/showCart etc remain unchanged.
  // If you want I can paste full rest of file - but main fixes were login + profile update flow.
