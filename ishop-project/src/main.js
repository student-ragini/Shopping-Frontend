...
$(function () {
  ...
  const API_BASE = (import.meta.env && import.meta.env.VITE_API_BASE_URL) ? import.meta.env.VITE_API_BASE_URL : '';
        ...
        .then(function (customers) {
          ...
          if (user && user.Password === formPwd) {
            ...
            if (typeof onSuccess === 'function') {
              ...
              $.ajax({ method: 'GET', url: 'shophome.html' })
                .then(function (resp) {
                  $('#bodyContainer').html(resp);
                });
            }
  ...
  $('#navCategoriesFooter').off('click').on('click', function (e) {
    e.preventDefault();
      ...
      .then(function (products) {
        const itemsWithProducts = cart.map(function (ci) {
          const prod = products.find(function (p) {
            return (p.id !== undefined && String(p.id) === String(ci.id)) ||
        ...
        const missingIds = itemsWithProducts
          .filter(function (it) { return !it.prod; })
          .map(function (it) { return String(it.cartId); });
        ...
        $('.btn-decrease').off('click').on('click', function () {
          ...
          let c = loadCartForCurrentUser();
          c.forEach(function (it) {
            if (String(it.id) === id) it.qty = Math.max(1, (it.qty || 1) - 1);
          });
          saveCartToLocalAndUser(c);
          updateCartCount();
          showCart();
    ...
    $.ajax({ method: 'GET', url: url })
      .then(function (response) {
        ...
        response.forEach(function (value) {
          ...
          const idVal = value.id !== undefined ? String(value.id) :
    ...
    $.ajax({ method: 'GET', url: API_BASE + '/getproducts' })
      .then(function (products) {
        const product = products.find(function (p) {
          return (p.id !== undefined && String(p.id) === String(productId)) ||
        ...
        const idVal = product.id !== undefined ? String(product.id) :
        ...
        const html =
          '<div class="container my-4">' +
            '<div class="row">' +
              '<div class="col-md-5">' +
                '<img src="' + fixImageUrl(image) + '" alt="' + title + '" ' +
