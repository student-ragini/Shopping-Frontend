...
$(function () {
  ...
  // 🔴 Yahi aapka Render backend URL hai
  const API_BASE = "https://shopping-backend-jb5p.onrender.com";
  // Agar kabhi local run karna ho to (sirf testing ke liye):
  ...
  // Normalize product image paths (images backend ke public folder se aayengi)
  function fixImageUrl(raw) {
    if (!raw) return "";
    try {
      // Absolute http(s) URL
      if (/^https?:\/\//i.test(raw)) {
        // Agar localhost / 127.0.0.1 hai to fname leke backend/public se serve kare
        if (/^(https?:\/\/)(127\.0\.0\.1|localhost)/i.test(raw)) {
          const fname = raw.split("/").pop();
          return fname ? API_BASE + "/public/" + fname : "";
        }
        return raw;
      }
      ...
      // "iphone.jpg", "/iphone.jpg", "img/iphone.jpg" → backend "/public/iphone.jpg"
      const fname = raw.split(/[\\/]/).pop();
      return fname ? API_BASE + "/public/" + fname : "";
    } catch (e) {
        ...
        $("#btnCheckout")
          ...
          .on("click", async function () {
            ...
            try {
              ...
              if (
                !window.confirm(
                  "You are about to place an order for ₹" + total + ". Proceed?"
                )
              ) {
                return;
              }
